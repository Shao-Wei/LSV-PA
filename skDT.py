...
def getData(poNum,target):
    file=open("samples/{}/{}.pla".format(target,poNum), 'r')
    nVar=0
    nSample=6400
    resultData=[]
    resultLabel=[]
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print('Missing argument. Usage: ./skDT.py <target> <nPo>')
        sys.exit()
    target = sys.argv[1]
    nPo = int(sys.argv[2])

    for poNum in range(nPo):
        logFile=open("samples/{}/skDT.csv".format(target),'w')
        ...
        sk2Blif(nVar, piNameList, poName, dt.best_estimator_, 'samples/{}/{}'.format(target,poNum))
